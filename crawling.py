...
with open('post_count.json', 'r', encoding='utf-8') as file:
    post_count = json.load(file)

#신규 id가 있다면 post_count.json에 추가
for id in account_id:
    if id not in post_count:
        post_count[id]=0

with open('post_count.json', 'w') as file:
...
print(len(reels_link))
print(len(texts))
print(reels_link[-1],texts[-1])
print("good crawling")